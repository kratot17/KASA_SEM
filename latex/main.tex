...
Pro zajištění reproducibility experimentu uvádíme klíčové parametry použité pro kódování. Je nutné zdůraznit, že parametr \hyperref[sec:glossary]{CRF} (Constant Rate Factor) není mezi různými kodéry přímo srovnatelný (např. \hyperref[sec:glossary]{CRF} 22 u x264 neprodukuje stejnou kvalitu jako \hyperref[sec:glossary]{CRF} 22 u \hyperref[sec:standards]{AV1}). Hlavním nástrojem pro srovnání efektivity jsou proto R-D křivky.
...
\begin{itemize}
    \item \textbf{\hyperref[sec:metrics]{PSNR} (Peak Signal-to-Noise Ratio):} Měří podíl mezi maximální hodnotou signálu a chybou (MSE). Ačkoliv je standardně používána, její korelace s lidským vnímáním je omezená, neboť nereflektuje sémantický obsah obrazu \cite{lec_iqa, wang2004image}.
...
Kromě průměrných hodnot jsme zkoumali i stabilitu kvality v čase. Jak ukazuje Obrázek \ref{fig:temporal_vmaf}, softwarové kodéry v režimu \hyperref[sec:glossary]{CRF} vykazují vysokou stabilitu, zatímco hardwarový kodér vykazuje propady v náročných pasážích, což je dáno zjednodušenou predikcí v \hyperref[sec:glossary]{ASIC} čipu.
